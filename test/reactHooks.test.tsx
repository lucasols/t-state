...
describe('useSlice', () => {
  ...
  const Component = ({ onRender }: { onRender?: anyFunction }) => {
    ...
    return (
      <button type="button" onClick={() => testState.setKey('key1', key1 + 1)}>
  ...
  test('update all components if setState is called from anywhere', () => {
    ...
    expect(button).toHaveTextContent('number: 42, string: 👍, array: [0,1,2]');
...
describe('useSelectorRC', () => {
  test('should not rerender when selected state does not change', () => {
    const testStore = new Store<{ key1: number; key2: number }>({
      state: { key1: 1, key2: 2 },
    });
    ...
    const { result } = renderHook(() => {
      ...
      return testStore.useSelectorRC(
        useCallback((state) => {
          return state.key1;
        }, []),
      );
    ...
    act(() => {
      testStore.setKey('key2', 3);
    });

    expect(renderCount).toBe(1);

    act(() => {
      testStore.setKey('key2', 4);
    });
  ...
  test('should rerender when selected state changes', () => {
    const testStore = new Store<{ key1: number; key2: number }>({
      state: { key1: 1, key2: 2 },
    });
